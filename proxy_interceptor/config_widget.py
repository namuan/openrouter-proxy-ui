...
def get_config_dir() -> Path:
    ...
    if system == "Windows":
        ...
        config_dir = Path(os.environ.get('APPDATA', Path.home() / 'AppData' / 'Roaming')) / 'OpenRouterProxy'
    elif system == "Darwin":
        ...
        config_dir = Path(os.environ.get('XDG_CONFIG_HOME', Path.home() / '.config')) / 'openrouter-proxy'
...
class ConfigWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        api_keys_help.setStyleSheet("color: gray; font-size: 11px;")
        ...
        api_models_help.setStyleSheet("color: gray; font-size: 11px;")
        api_models_layout.addWidget(api_models_help)
        
        self.api_models_text = QTextEdit()
        self.api_models_text.setPlaceholderText("qwen/qwen3-coder:free\nopenai/gpt-oss-20b:free\nanthropic/claude-3-haiku:beta")
        ...
        auth_tokens_help = QLabel("Enter allowed authentication tokens, one per line (leave empty to disable auth):")
        auth_tokens_help.setStyleSheet("color: gray; font-size: 11px;")
    ...
    def _parse_config(self):
        ...
        self.api_models = [model.strip() for model in api_models_text.split('\n') if model.strip()]
        
        # Parse auth tokens
        auth_tokens_text = self.auth_tokens_text.toPlainText().strip()
        self.auth_tokens = {token.strip() for token in auth_tokens_text.split('\n') if token.strip()}
    ...
    def _load_config(self):
        ...
        try:
            ...
            logger.debug(f"Raw saved_models from config: {saved_models}, type: {type(saved_models)}")
            ...
            logger.debug(f"Loaded {len(self.api_keys)} API keys, {len(self.api_models)} models, {len(self.auth_tokens)} auth tokens")
            ...
            logger.debug(f"Default config - {len(self.api_keys)} API keys, {len(self.api_models)} models")
            logger.debug(f"Valid config check: {self.has_valid_config()}")
